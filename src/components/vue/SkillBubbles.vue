...
<template>
  <div class="skill-map" aria-label="Skills and technologies">
    <!-- Floating scatter layout (md+) -->
    <div class="skill-map__scatter" aria-hidden="true">
      <span
        v-for="skill in skills"
        :key="skill.label"
        :class="['skill-bubble', `skill-bubble--${skill.size}`, `skill-bubble--${skill.accent}`]"
    ...
    <div class="skill-map__mobile" aria-hidden="true">
      <span
        v-for="skill in skills"
        :key="`mob-${skill.label}`"
        :class="['skill-bubble', `skill-bubble--sm`, `skill-bubble--${skill.accent}`]"
...
<style lang="scss" scoped>
  ...
  /* Mobile fallback: simple flex-wrap pill row */
  .skill-map__mobile {
    display: flex;
    flex-wrap: wrap;
    gap: 10px;
